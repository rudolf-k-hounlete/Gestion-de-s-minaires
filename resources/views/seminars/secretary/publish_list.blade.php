{{-- resources/views/seminars/secretary/publish_list.blade.php --}}
@extends('layouts.app')
...
<head>
    ...
    <title>Séminaires à publier | IMSP Séminaires</title>
...
<body>
    ...
    <div class="container">
        <div class="header">
            <h1>Séminaires à publier</h1>
            <p>Gérez et publiez les séminaires programmés</p>
        </div>

        @if($seminars->isEmpty())
            <div class="empty-state">
                <i class="fas fa-calendar-slash"></i>
                <h2>Aucun séminaire à publier</h2>
                <p>Il n’y a aucun séminaire à publier pour le moment.</p>
            </div>
        @else
            <div class="seminar-grid">
                @foreach($seminars as $seminar)
                    @php
                        ...
                        $allowedDate   = $scheduledDate->copy()->subDays(7)->startOfDay();
                    ...
                    <div class="seminar-card">
                        ...
                        <div class="detail-item">
                            ...
                            @if(!empty(trim($seminar->summary ?? '')))
                                <a href="{{ route('seminars.secretary.showSummary', $seminar->id) }}" 
                                   class="action-btn">
                                    <i class="fas fa-file-alt"></i> Voir le résumé
                                </a>
                            @else
                                <span class="text-gray-500 text-sm italic">Aucun résumé disponible</span>
                            @endif
                        </div>
                        
                        <div class="card-actions">
                            @if($today->equalTo($allowedDate))
                                <form method="POST" action="{{ route('seminars.secretary.publish', $seminar->id) }}">
                                    @csrf
                                    <input type="hidden" name="scheduled_date" value="{{ $seminar->scheduled_date }}">
                                    <button type="submit" class="btn-publish">
                                        <i class="fas fa-upload"></i> Publier maintenant
                                    </button>
                                </form>
                            @else
                                <button type="button" disabled class="btn-disabled">
                                    <i class="fas fa-clock"></i> Publier pour le {{ $scheduledDate->format('d/m/Y') }}
                                </button>
                            @endif
                        </div>
                    </div>
